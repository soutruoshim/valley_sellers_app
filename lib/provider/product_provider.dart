  ...
  Future <void> initSellerProductList(String sellerId, int offset, BuildContext context, String languageCode,String search, {bool reload = true}) async {
    if(reload) {
      ...
      if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
        ...
        _sellerProductList.addAll(_sellerProductModel.products);
        _sellerPageSize = ProductModel.fromJson(apiResponse.response.data).totalSize;
        _firstLoading = false;
        _isLoading = false;
        if(_sellerProductList.length != 0){
          for(int index = 0; index < _sellerProductList.length; index++) {
            _reviewProductIds.add(_sellerProductList[index].id);
          }
          _reviewProductIndex = _reviewProductIds[0];
        }
      } else {
